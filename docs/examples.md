...
Let's build an API Client using cicerone.

Our [GitHub](https://github.com/phalt/cicerone/tree/main/example_openapi_specs) has a bunch of schemas that are proven to work with cicerone, so let's use one of those!
...
```sh
cicerone generate -u https://raw.githubusercontent.com/phalt/cicerone/main/example_openapi_specs/best.json -o my_client/
```
...
cicerone also supports the major HTTP methods PUT and DELETE in the same way.

### URL and Query parameters

If your endpoint takes [path parameters](https://learn.openapis.org/specification/parameters.html#parameter-location) (aka URL parameters) then cicerone will turn them into parameters in the function:
...
Query parameters will also be generated the same way. See [this example](https://github.com/phalt/cicerone/blob/0.4.4/tests/test_client/client.py#L71) for a function that takes a required query parameter.

Note that, optional parameters that are not passed will be omitted when the URL is generated by cicerone.
...
cicerone keeps a mapping of the paths and their potential response codes. When it gets a response code that fits into the map, it generates the pydantic object associated to it.
...
cicerone solves this problem by providing an _entry point_ for configuration that will never be overwritten - `config.py`.
...
Then cicerone will provide you information on the environment variables you need to set to
