...
This document describes the workflow for publishing cicerone to Homebrew.

## Prerequisites

1. Have a new version ready to release
2. Version must be updated in `pyproject.toml` and `cicerone/settings.py`
...
This will create `homebrew/cicerone.rb` with:
...
```bash
# Create a new repository on GitHub named: homebrew-cicerone
# Clone it locally
git clone https://github.com/phalt/homebrew-cicerone.git
cd homebrew-cicerone

# Initialize the structure
mkdir -p Formula
cp /path/to/cicerone/homebrew/cicerone.rb Formula/cicerone.rb
git add Formula/cicerone.rb
git commit -m "Add cicerone formula v0.9.0"
git push
```
...
cp /path/to/cicerone/homebrew/cicerone.rb /path/to/homebrew-cicerone/Formula/cicerone.rb

# Commit and push
cd /path/to/homebrew-cicerone
git add Formula/cicerone.rb
git commit -m "Update cicerone to version X.Y.Z"
git push
```

### 6. Announce the Release

Users can now install cicerone via Homebrew:

```bash
brew tap phalt/cicerone
brew install cicerone
```

Or in a single command:

```bash
brew install phalt/cicerone/cicerone
```
...
jobs:
  update-formula:
    ...
    steps:
      ...
      - name: Update tap
        run: |
          # Push to homebrew-cicerone repository
